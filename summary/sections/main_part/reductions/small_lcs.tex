...
We further require $\alpha_M = 2$ at first.\footnote{See \autoref{sec:small_lcs:matching_pairs_reduction} for the reason of the restriction and how to lift it.} %, i.e., the number of deleted symbols in the smaller string is asymptotically the same as its length.
...
\paragraph*{Reduction}
We now use \autoref{thm:small_lcs_construction} to instantiate the actual reduction. %with the desired bounds.
...
We show this for a reduced parameter set of $P' = \{n, m, L, \Sigma, M\}$.\footnote{For a proof of the full parameter set we refer to the original paper \cite[section 9.1.2]{Bringman.2018}.} %for the current case.
